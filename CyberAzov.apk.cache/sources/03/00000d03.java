...
public final class JsonKeysetReader implements KeysetReader {
    ...
    private EncryptedKeyset encryptedKeysetFromJson(JSONObject json) throws JSONException {
        ...
        if (this.urlSafeBase64) {
            bArr = Base64.urlSafeDecode(json.getString("encryptedKeyset"));
        } else {
            bArr = Base64.decode(json.getString("encryptedKeyset"));
        }
        return EncryptedKeyset.newBuilder().setEncryptedKeyset(ByteString.copyFrom(bArr)).setKeysetInfo(keysetInfoFromJson(json.getJSONObject("keysetInfo"))).build();
    }
    ...
    private KeyData keyDataFromJson(JSONObject json) throws JSONException {
        ...
        if (this.urlSafeBase64) {
            bArr = Base64.urlSafeDecode(json.getString("value"));
        } else {
            bArr = Base64.decode(json.getString("value"));
        }
        return KeyData.newBuilder().setTypeUrl(json.getString("typeUrl")).setValue(ByteString.copyFrom(bArr)).setKeyMaterialType(getKeyMaterialType(json.getString("keyMaterialType"))).build();
    }
