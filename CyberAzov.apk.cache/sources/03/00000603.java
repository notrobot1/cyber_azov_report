...
public final class PrfAesCmac implements Prf {
    ...
    public byte[] compute(final byte[] data, int outputLength) throws GeneralSecurityException {
        ...
        if (max * 16 == data.length) {
            bArr = Bytes.xor(data, (max - 1) * 16, this.subKey1, 0, 16);
        } else {
            bArr = Bytes.xor(AesUtil.cmacPad(Arrays.copyOfRange(data, (max - 1) * 16, data.length)), this.subKey2);
        }
        byte[] bArr2 = new byte[16];
        for (int i = 0; i < max - 1; i++) {
            bArr2 = instance.doFinal(Bytes.xor(bArr2, 0, data, i * 16, 16));
        }
        return Arrays.copyOf(instance.doFinal(Bytes.xor(bArr, bArr2)), outputLength);
    }
