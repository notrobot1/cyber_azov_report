...
public abstract class DispatchedTask<T> extends Task {
    ...
    public final void run() {
        ...
        try {
            ...
            Job job = (exceptionalResult$kotlinx_coroutines_core != null || !DispatchedTaskKt.isCancellableMode(this.resumeMode)) ? null : (Job) context2.get(Job.Key);
            if (job != null && !job.isActive()) {
                ...
                if (DebugKt.getRECOVER_STACK_TRACES() && (continuation instanceof CoroutineStackFrame)) {
                    ...
                    continuation.resumeWith(Result.m48constructorimpl(ResultKt.createFailure(cancellationException)));
                }
                cancellationException = cancellationException2;
                continuation.resumeWith(Result.m48constructorimpl(ResultKt.createFailure(cancellationException)));
            } else if (exceptionalResult$kotlinx_coroutines_core != null) {
                Result.Companion companion2 = Result.Companion;
                continuation.resumeWith(Result.m48constructorimpl(ResultKt.createFailure(exceptionalResult$kotlinx_coroutines_core)));
            } else {
                T successfulResult$kotlinx_coroutines_core = getSuccessfulResult$kotlinx_coroutines_core(takeState$kotlinx_coroutines_core);
                Result.Companion companion3 = Result.Companion;
                continuation.resumeWith(Result.m48constructorimpl(successfulResult$kotlinx_coroutines_core));
            }
            ...
            try {
                ...
                obj2 = Result.m48constructorimpl(Unit.INSTANCE);
            } catch (Throwable th) {
                Result.Companion companion5 = Result.Companion;
                obj2 = Result.m48constructorimpl(ResultKt.createFailure(th));
            }
            handleFatalException(null, Result.m51exceptionOrNullimpl(obj2));
        } catch (Throwable th2) {
            try {
                Result.Companion companion6 = Result.Companion;
                DispatchedTask<T> dispatchedTask2 = this;
                taskContext.afterTask();
                obj = Result.m48constructorimpl(Unit.INSTANCE);
            } catch (Throwable th3) {
                Result.Companion companion7 = Result.Companion;
                obj = Result.m48constructorimpl(ResultKt.createFailure(th3));
            }
            handleFatalException(th2, Result.m51exceptionOrNullimpl(obj));
        }
    }
