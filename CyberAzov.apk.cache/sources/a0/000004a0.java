...
public final class ViewingConditions {
    ...
    static ViewingConditions make(float[] fArr, float f, float f2, float f3, boolean z) {
        ...
        if (f8 >= 0.9d) {
            f4 = MathUtils.lerp(0.59f, 0.69f, (f8 - 0.9f) * 10.0f);
        } else {
            f4 = MathUtils.lerp(0.525f, 0.59f, (f8 - 0.8f) * 10.0f);
        }
        float f9 = f4;
        float exp = z ? 1.0f : (1.0f - (((float) Math.exp(((-f) - 42.0f) / 92.0f)) * 0.2777778f)) * f8;
        double d = exp;
        if (d > 1.0d) {
            exp = 1.0f;
        } else if (d < 0.0d) {
        ...
        float f12 = 1.0f - f11;
        float cbrt = (f11 * f) + (0.1f * f12 * f12 * ((float) Math.cbrt(f * 5.0d)));
        ...
        return new ViewingConditions(yFromLstar, ((fArr5[0] * 2.0f) + fArr5[1] + (fArr5[2] * 0.05f)) * pow, pow, pow, f9, f8, fArr3, cbrt, (float) Math.pow(cbrt, 0.25d), sqrt);
    }
