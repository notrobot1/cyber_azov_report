...
public final class EllipticCurves {
    ...
    private static byte[] toMinimalSignedNumber(byte[] bs) {
        ...
        if ((bs[i2] & ByteCompanionObject.MIN_VALUE) == 128) {
            i = 1;
        }
        byte[] bArr = new byte[(bs.length - i2) + i];
        System.arraycopy(bs, i2, bArr, i, bs.length - i2);
        return bArr;
    }
    ...
    public static byte[] ecdsaDer2Ieee(byte[] der, int ieeeLength) throws GeneralSecurityException {
        ...
        System.arraycopy(der, i7 + i, bArr, (ieeeLength - i8) + i, i8 - i);
        return bArr;
    }
    ...
    public static boolean isValidDerEncoding(final byte[] sig) {
        ...
        if (sig.length >= 8 && sig[0] == 48) {
            int i2 = sig[1] & UByte.MAX_VALUE;
            if (i2 == 129) {
                i2 = sig[2] & UByte.MAX_VALUE;
                if (i2 < 128) {
                    return false;
                }
                i = 2;
            ...
            int i4 = i3 + 1;
            int i5 = sig[i4] & UByte.MAX_VALUE;
            int i6 = i4 + 1 + i5 + 1;
            if (i6 >= sig.length || i5 == 0) {
                return false;
            }
            int i7 = i + 3;
            if ((sig[i7] & UByte.MAX_VALUE) >= 128) {
                return false;
            }
            if ((i5 > 1 && sig[i7] == 0 && (sig[i + 4] & UByte.MAX_VALUE) < 128) || sig[i7 + i5] != 2) {
                return false;
            }
            int i8 = sig[i6] & UByte.MAX_VALUE;
            if (i6 + 1 + i8 != sig.length || i8 == 0) {
                return false;
            }
            int i9 = i + 5 + i5;
            if ((sig[i9] & UByte.MAX_VALUE) >= 128) {
                return false;
            }
            return i8 <= 1 || sig[i9] != 0 || (sig[(i + 6) + i5] & UByte.MAX_VALUE) >= 128;
        }
        return false;
    }
